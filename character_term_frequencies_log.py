...
relative_frequencies = {word: word_sum ** 1.2 / word_totals[word] for word, word_sum in word_sums.items()
...
for i in range(min(20, len(sorted_relative_frequencies))):
    ...
    print(f'#{i + 1:<2} {", ".join(words)} ({frequency:0.5f} : '
          f'{", ".join([f"{word_sums[word]:>0.4f}/{word_totals[word]:>0.4f}" for word in words])})')
